...
{
	public partial class frm_clientes_agregar : Form
	{

		Clientes Clie;
		frm_clientes FormClientes;

		public frm_clientes_agregar(frm_clientes FrmClientes)
		{
			InitializeComponent();
			Clie = new Clientes();
		...
		private void btn_add_agregar_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else
				{
					...
					Clie.InsertarClientes();
					FormClientes.RefreshGrid();
					LimpiarCampos();
					frm_facturacion FormFacturacion = new frm_facturacion();
					FormFacturacion.OpcionesDefecto();
					MessageBox.Show("El cliente ha sido añadido con exito", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
			}
			catch (Exception ex)
			{
